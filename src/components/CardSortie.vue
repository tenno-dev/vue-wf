      ...
      <v-card style="width: 100%;">
        ...
        <v-list dense subheader>
          ...
          <v-list-tile>
            ...
            <v-list-tile-action>
              <v-list-tile-action-text>
                <v-chip style="right: 0px;" small :color="getcolourfaction(sortie.faction)" text-color="white">
                  {{sortie.faction}}
                </v-chip>
                <br>
              </v-list-tile-action-text>
              ...
              <v-list-tile-action>
                <v-list-tile-action-text>
                  <v-chip style="right: 0px;" small color="grey" text-color="white">
                    {{nodename(variant.node)}} on {{planetname(variant.node)}}
                  </v-chip>
                  <br>
                </v-list-tile-action-text>
              </v-list-tile-action>
            ...
            <v-list-tile>
              ...
              <v-list-tile-action>
                <v-list-tile-action-text>
                  <v-chip style="right: 0px;" small color="grey" text-color="white">
                    <v-avatar>
                      <v-icon v-if="variant.missionType ==='Spy'" class="grey darken-4">mdi-incognito</v-icon>
                      <v-icon v-else-if="variant.missionType ==='Rescue'" class="red darken-4">mdi-lifebuoy</v-icon>
                      <v-icon v-else-if="variant.missionType ==='Defense' | variant.missionType ==='Mobile Defense'" class="blue darken-4">mdi-shield</v-icon>
                      <v-icon v-else-if="variant.missionType ==='Capture'" class="grey darken-4">mdi-target</v-icon>
                      <v-icon v-else-if="variant.missionType ==='Excavation'" class="green darken-4">mdi-shovel</v-icon>
